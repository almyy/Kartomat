...
test.describe("Edge Cases", () => {
  ...
  test("should handle a full classroom (all seats filled)", async ({
    page,
  }) => {
    await configureClassroom(page, 2, 2);
    ...
    const filledSeats = await seatingDisplay
  ...
  test("should remove constraints when clicking remove button", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText("Alice og Bob skal IKKE sitte sammen"),
    ).toBeVisible();
    // Remove the constraint
    await page
      .getByRole("button", { name: "Fjern begrensning" })
      .click({ force: true });
  ...
  test("should handle empty classroom with students but no solve", async ({
    page,
  }) => {
    await configureClassroom(page, 3, 3);
    ...
    await expect(
      page.getByText("Legg til elever og begrensninger"),
    ).toBeVisible();
  });
});
