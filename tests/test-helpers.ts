 ...
 */
export async function configureClassroom(
  page: Page,
  rows: number,
  cols: number,
) {
 ...
 */
export async function addNotTogetherConstraint(
  page: Page,
  student1: string,
  student2: string,
  exact?: boolean,
 ...
 */
export async function addTogetherConstraint(
  page: Page,
  student1: string,
  student2: string,
  exact?: boolean,
 ...
 */
export async function addRowConstraint(
  page: Page,
  student: string,
  row: number,
  exact?: boolean,
  ...
  await constraintSection
    .getByRole("textbox", { name: "Radnummer" })
    .fill(String(row));
 ...
 */
export async function addAbsoluteConstraint(
  page: Page,
  student: string,
  row: number,
  col: number,
 ...
 */
export async function addFarApartConstraint(
  page: Page,
  student1: string,
  student2: string,
  minDistance: number,
  ...
  const constraintSection = page.getByRole("region", { name: "Begrensninger" });
  await constraintSection
    .getByRole("textbox", { name: "Seter" })
    .fill(String(minDistance));
...
export async function solveSeating(page: Page) {
  ...
  await page.waitForSelector(".bg-indigo-600\\/30, .bg-red-500\\/20", {
    timeout: 10000,
  });
}
...
export async function verifyStudentInSeating(page: Page, studentName: string) {
  ...
  const studentSeat = seatingDisplay
    .locator(".flex-1.min-h-\\[50px\\]")
    .filter({ hasText: new RegExp(`^${studentName}$`) });
...
export async function getStudentPosition(page: Page, studentName: string) {
  ...
  for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
    const seats = await rows[rowIndex]
      .locator(".flex-1.min-h-\\[50px\\]")
      .all();
