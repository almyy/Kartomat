...
import { addStudents, configureClassroom, solveSeating, verifyStudentInSeating, getStudentPosition } from './test-helpers';
...
test.describe('Classroom Configuration', () => {
  ...
  test('should solve a simple 2x2 classroom with 3 students', async ({ page }) => {
  ...
  test('should solve a 4x4 classroom with 10 students', async ({ page }) => {
    ...
    await addStudents(page, ['Alice', 'Bob', 'Charlie', 'David', 'Eve', 'Frank', 'Grace', 'Henry', 'Ivy', 'Jack']);
    
    await solveSeating(page);
    
    // Verify all students are placed
    for (const name of ['Alice', 'Bob', 'Charlie', 'David', 'Eve', 'Frank', 'Grace', 'Henry', 'Ivy', 'Jack']) {
  ...
  test('should handle 10x10 large classroom', async ({ page }) => {
    ...
    await addStudents(page, ['A1', 'A2', 'A3', 'A4', 'A5', 'B1', 'B2', 'B3', 'B4', 'B5']);
    
    await solveSeating(page);
    
    // Verify all students are placed
    for (const name of ['A1', 'A2', 'A3', 'A4', 'A5', 'B1', 'B2', 'B3', 'B4', 'B5']) {
      await verifyStudentInSeating(page, name);
    }
  });
});
