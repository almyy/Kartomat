...
test.describe("Absolute Placement Constraint", () => {
  ...
  test('should handle "Absolute Placement" constraint', async ({ page }) => {
    ...
    await expect(
      page.getByText("Alice må sitte på rad 1, kolonne 2"),
    ).toBeVisible();
  ...
  test("should prioritize absolute constraint over row constraint", async ({
    page,
  }) => {
    await configureClassroom(page, 3, 3);
    ...
    await selectMantineOption(
      page,
      "Begrensningstype",
      /Må være i rad|must be in row/i,
    );
  ...
  test("should work with absolute and together constraints", async ({
    page,
  }) => {
    await configureClassroom(page, 3, 3);
    await addStudents(page, ["Alice", "Bob", "Charlie"]);
    ...
    await selectMantineOption(
      page,
      "Begrensningstype",
      /Må være sammen|Must be together/i,
    );
  ...
  test("should handle absolute constraint at corner positions", async ({
    page,
  }) => {
    await configureClassroom(page, 3, 3);
