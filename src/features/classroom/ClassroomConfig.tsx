...
export function ClassroomConfig() {
  ...
  const availableSeats = seatState
    .flat()
    .filter((seat) => seat !== "off").length;
  ...
  const getSeatTitle = (state: SeatState) => {
    ...
    if (state === "m")
      return `${t("classroom.availableSeat")} - ${t("students.genderMale")}`;
    if (state === "f")
      return `${t("classroom.availableSeat")} - ${t("students.genderFemale")}`;
  ...
  return (
    <Accordion.Item value="classroom">
      ...
      <Accordion.Panel>
        ...
        <div className="mb-3 sm:mb-4">
          <div className="flex justify-between items-center mb-2">
            <Text size="sm" c="dimmed">
              {t("classroom.genderInstruction")}
            </Text>
            <Button onClick={alternateGenders} size="xs" color="grape">
        ...
        <Text size="sm" c="dimmed">
          {t("classroom.availableSeats", { count: availableSeats })}
        </Text>
      </Accordion.Panel>
    </Accordion.Item>
  );
